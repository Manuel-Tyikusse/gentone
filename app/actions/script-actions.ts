...
export async function getUserProfile(): Promise<UserProfileResponse> {
  try {
    ...
    // Tenta encontrar o perfil. Se não existir, cria um com 10 créditos e plano Free
    const profile = await collection.findOneAndUpdate(
      { userId: userId },
      { 
        $setOnInsert: { 
...
/**
 * GERA O ROTEIRO REAL USANDO A IA (GROQ)
 */
export async function generateScriptAction(formData: { 
  topic: string, 
  tone: string, 
  ...
  try {
    ...
    // Verifica créditos antes de gastar a API
    const profile = await db.collection("profiles").findOne({ userId: userId });
    if (!profile || profile.credits <= 0) {
      return { success: false, error: "Insufficient credits!" };
    }

    // GENTONE V10.1 - THE GLOBAL ELITE PROTOCOL (Restaurado)
    const systemInstruction = `
      [ROLE]: You are GenTone, a world-class Viral Content Strategist for professional creators.
